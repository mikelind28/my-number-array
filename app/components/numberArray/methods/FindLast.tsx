...
export default function FindLast() {
    ...
    useEffect(() => {
        ...
        if (selectValue === "<") {
            const result = myNumberArray.findLast((value: number) => value < parseInt(inputValue));
    ...
    return (
        <>
            <title>myNumberArray | .findLast() Method</title>
            <p>
                The <code>.findLast()</code> array method takes a test function as a parameter. It returns the last item in the array that satisfies the test function. If no value satisfies the function, it returns <code>undefined</code>.
            </p>
            ...
            <div className="flex flex-col gap-2 my-2 p-2 bg-gray-950/75 rounded-sm">
                <div className="flex flex-wrap items-baseline leading-6">
                    <code>myNumberArray.findLast(</code>
                    <code>(value){` => {`}</code>
                </div>
